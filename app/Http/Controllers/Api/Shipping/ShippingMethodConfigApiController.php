...
class ShippingMethodConfigApiController extends Controller
{
    ...
    /**
     * Get all configs for a shipping method (merged provider + method-specific)
    ...
    public function index($shippingMethodId)
    {
        try {
            ...
            $configs = $this->shippingMethodRepository->getConfig($shippingMethodId);

            // Get shipping method info to show provider details
            $data = [
                'shipping_method' => [
     ...
     */
    public function show($shippingMethodId, $key)
    {
        try {
            $shippingMethod = $this->shippingMethodRepository->findById($shippingMethodId);
            ...
            $value = $this->shippingMethodRepository->getConfig($shippingMethodId, $key);
     ...
     */
    public function update(Request $request, $shippingMethodId, $key)
    {
        try {
            $shippingMethod = $this->shippingMethodRepository->findById($shippingMethodId);
            ...
            $updatedValue = $this->shippingMethodRepository->getConfig($shippingMethodId, $key);
     ...
     */
    public function bulkUpdate(Request $request, $shippingMethodId)
    {
        try {
            $shippingMethod = $this->shippingMethodRepository->findById($shippingMethodId);
            ...
            // Get updated configs
            $updatedConfigs = $this->shippingMethodRepository->getConfig($shippingMethodId);
     ...
     */
    public function destroy($shippingMethodId, $key)
    {
        try {
            $shippingMethod = $this->shippingMethodRepository->findById($shippingMethodId);
            ...
            $this->shippingMethodRepository->deleteConfigOverride($shippingMethodId, $key);
    ...
    /**
     * Get all global configs (list all shipping_method_configs)
     * GET /api/shipping-method-configs
     */
    public function getAllGlobalConfigs()
    {
        try {
            $globalConfigs = DB::table('shipping_method_configs')
                ->orderBy('provider')
                ->get();

            $resultData = $globalConfigs->map(function($config) {
                // Get config items for this global config
                $items = DB::table('shipping_method_config_items')
                ...
                // Get count of shipping methods using this config
     ...
     * GET /api/shipping-method-configs/{id}
     */
    public function getGlobalConfig($configId)
    {
        try {
            ...
            // Get shipping methods using this config
     ...
     * POST /api/shipping-method-configs
     */
    public function createGlobalConfig(Request $request)
    {
        try {
            $validated = $request->validate([
                'name' => 'required|string|max:100|unique:shipping_method_configs,name',
            ...
            $configId = DB::table('shipping_method_configs')->insertGetId([
                'name' => $validated['name'],
            ...
            foreach ($validated['configs'] as $configItem) {
                ...
                DB::table('shipping_method_config_items')->insert([
                    'shipping_method_config_id' => $configId,
                    'key' => $configItem['key'],
                    'value' => $value,
     ...
     * PUT /api/shipping-method-configs/{id}
     */
    public function updateGlobalConfig(Request $request, $configId)
    {
        try {
            $validated = $request->validate([
                'name' => 'sometimes|string|max:100|unique:shipping_method_configs,name,' . $configId,
                'provider' => 'sometimes|string|max:50',
                'description' => 'nullable|string',
                'configs' => 'sometimes|array|min:1',
            ]);

            $config = DB::table('shipping_method_configs')->where('id', $configId)->first();
            if (!$config) {
                $result = (new ResultBuilder())
                    ->setStatus(false)
                    ->setStatusCode('404')
            ...
            if (!empty($updateData)) {
                ...
                DB::table('shipping_method_configs')->where('id', $configId)->update($updateData);
            }
            ...
            if (isset($validated['configs'])) {
                ...
                foreach ($configsArray as $configItem) {
                    ...
                    $existing = DB::table('shipping_method_config_items')
                        ->where('shipping_method_config_id', $configId)
                        ->where('key', $configItem['key'])
                        ->first();

                    if ($existing) {
                        // Update existing
                        DB::table('shipping_method_config_items')
     ...
     * DELETE /api/shipping-method-configs/{id}
     */
    public function deleteGlobalConfig($configId)
    {
        try {
            ...
            // Check if any shipping methods are using this config
            $methodCount = DB::table('shipping_methods')
                ->where('shipping_method_config_id', $configId)
                ->count();
            ...
            if ($methodCount > 0) {
                $result = (new ResultBuilder())
                    ->setStatus(false)
                    ->setStatusCode('400')
                    ->setMessage('Cannot delete global config that is being used by ' . $methodCount . ' shipping method(s). Please unassign methods first.')
                    ->setData([]);
            ...
            // Delete global config
            DB::table('shipping_method_configs')->where('id', $configId)->delete();
     ...
     * DELETE /api/shipping-method-configs/{id}/items/{key}
     */
    public function deleteGlobalConfigItem($configId, $key)
    {
        try {
            ...
            $deleted = DB::table('shipping_method_config_items')
                ->where('shipping_method_config_id', $configId)
                ->where('key', $key)
                ->delete();
