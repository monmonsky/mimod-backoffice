...
$(document).ready(function() {
    ...
    $form.on('submit', async function(e) {
        ...
        try {
            const response = await fetch('/access-control/role/store', {
                method: 'POST',
                headers: {
            ...
            if (response.ok && result.success) {
                ...
                if (result.errors) {
                    ...
                    Toast.showToast(result.message || 'Failed to create role', 'error');
                }
                $submitBtn.prop('disabled', false).html(originalBtnText);
            }
        } catch (error) {
            console.error('Error creating role:', error);
            Toast.showToast('An error occurred while creating role', 'error');
            $submitBtn.prop('disabled', false).html(originalBtnText);
        }
    });
});
